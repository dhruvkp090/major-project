...
def predict(home_team, away_team, city, toss_winner, toss_decision):
    ...
    matches_df = matches_cleaned_data[['team1', 'team2', 'city', 'toss_winner',
                                       'toss_decision', 'Strength Team 1', 'Strength Team 2', 'winner']]
    ...
    results = convert_to_numerical_field(
        home_team, away_team, city, toss_winner, toss_decision)
    results.append(strengths[home_team])
    results.append(strengths[away_team])
    print(results)
    predictions = classifier.predict([results])
